export default function ServiceCard({ servicio, action }) {
    ...
    <div
      ...
      className={`${
        servicio.status === "Approved"
          ...
          ? "bg-yellow-300"
          : "bg-gray-200"
      } w-[90%] mt-3 rounded-lg grid grid-cols-[60%_40%] hover:cursor-pointer hover:scale-105 ease-in-out duration-100`}
    >
      <div className="flex flex-col ml-2 justify-center">
        <span className="font-semibold">
          {ServicioHecho || "Sin descripción"}
        </span>
        <span className="hidden sm:block font-semibold">
          {formatearFecha(creado)}
        </span>
      </div>
